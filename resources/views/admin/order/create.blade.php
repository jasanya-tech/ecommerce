                            ...
                            <div class="card-body">
                                <form method="POST" action="{{ route('order.store') }}" enctype="multipart/form-data">
                                    ...
                                    <div id="selected-products" class="mb-3">
                                        ...
                                        @if (old('selected_products'))
                                            @foreach (json_decode(old('selected_products')) as $selectedProduct)
                                        ...
                                        <select class="form-control" id="payment" name="payment" required>
                                            ...
                                            @foreach ($payments as $payment)
                                                <option value="{{ $payment->id }}"
                                                    @if (old('payment') == $payment->id) selected @endif>
                                                    {{ $payment->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="status" name="status" required>
                                            ...
                                            <option value="Pesanan Dikirim"
                                                @if (old('status') == 'Pesanan Dikirim') selected @endif>Pesanan Dikirim</option>
                                            <option value="Pesanan Diterima"
                                                @if (old('status') == 'Pesanan Diterima') selected @endif>Pesanan Diterima</option>
                                            <option value="Pesanan Dibatalkan"
                                                @if (old('status') == 'Pesanan Dibatalkan') selected @endif>Pesanan Dibatalkan
                                            </option>
                                        </select>
                                        @error('status')
                                            <span class="help-block" style="color:red;">{{ $message }}</span>
